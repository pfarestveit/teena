...
begin
  ...
  courses_heading = %w(Term CCN Course Title Format Units)
  courses_csv = Utils.create_test_output_csv('boac-class-page-courses.csv', courses_heading)

  meetings_heading = %w(Term CCN Course Instructors Days Time Location)
                  ...
                  unless api_course[:code].include? 'PHYS ED'
                    ...
                    sections.each do |section|
                      begin
                        ...
                        unless api_section_page.student_uids.length > BOACUtils.config['class_page_max_size']
                          expected_students = api_section_page.student_uids.map { |u| BOACUser.new uid: u }
                          all_student_data = []
