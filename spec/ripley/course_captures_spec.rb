        ...
        if (course_sites = data['tests']['course_capture'])
          ...
          course_sites.each do |course_site|
            ...
            if expected_sections
              ...
              recordings_exist = @course_captures_page.verify_block do
                @course_captures_page.wait_until(Utils.medium_wait) { @course_captures_page.section_content_elements.any? }
              end
              ...
              if recordings_exist
                ...
                expected_sections.each do |section|
                  ...
                  if expected_sections.length > 1
                    it "shows UID #{user.uid} all the available lecture videos for one of #{expected_sections.length} primary sections on site ID #{course.site_id}" do
                      expect(visible_video_count).to eql(expected_video_count)
                    end
                    it "shows UID #{user.uid} the section code '#{expected_section_code}' on site ID #{course.site_id}" do
                      expect(visible_section_code).to eql(expected_section_code)
                    end
                  else
                    it "shows UID #{user.uid} all the available lecture videos for the one primary section on site ID #{course.site_id}" do
                      expect(visible_video_count).to eql(expected_video_count)
                    end
                    it "shows UID #{user.uid} no section code on site ID #{course.site_id}" do
                      expect(visible_section_code).to be nil
                    end
                  end
                  ...
                  if data == tests.first
                    ...
                    it "shows UID #{user.uid} an explanation for viewing the recordings at You Tube on site ID #{course.site_id}" do
                      expect(has_you_tube_alert).to be_truthy
                    end
                    it "shows UID #{user.uid} a 'help page' link on site ID #{course.site_id}" do
                      expect(has_help_page_link).to be true
                    end
                    it "shows UID #{user.uid} a valid link to YouTube video ID #{expected_video_id} on site ID #{course.site_id}" do
                      expect(has_you_tube_link).to be true
                    end
                    ...
                    it "offers UID #{user.uid} a 'Report a Problem' link on on site ID #{course.site_id}" do
